...
Discord bot for beautifying, decoding, and extracting from Lua scripts.

## Commands

- `/help` — show command list
- `/beautify` — clean up Lua formatting
- `/decode` — decode hex/decimal escapes, string.char, fold concats
- `/extract` — pull strings, URLs, and loadstring payloads

Each command accepts `code` (paste), `file` (upload), or `url` (raw link).
...
async function getSource(interaction) {
  ...
  if (file) {
    ...
    if (file.size > MAX_BYTES) return { error: `File too large (${(file.size/1024).toFixed(1)} KB, max ${MAX_BYTES/1024} KB)` };
    try {
      ...
      if (cmd === 'help') {
        const embed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: '/decode', value: 'Decode \\x escapes, \\ddd escapes, string.char, fold concats' },
